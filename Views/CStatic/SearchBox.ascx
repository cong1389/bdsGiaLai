...
<div class="box-timkiem all">
    <div class="in-tab">
        ...
        <a class="a-btl-uload" href="<%=ViewPage.GetURL(WebLogin.IsLogin() == true ? "dang-tin" : "dang-nhap")%>" style="background: url(&quot;images/icon-1.png&quot;) no-repeat scroll 51px 12px #FF9900;">Đăng Tin</a>
        ...
        <form onsubmit="doSearch(); return false;">
            ...
            <p id="list_Price">
                <select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDB != null && i < listPriceNDB.Count; i++){%><option value="<%=listPriceNDB[i].ID %>" <%if (listPriceNDB[i].ID == PriceID){%>selected<%} %>><%=listPriceNDB[i].Name %></option> <%} %></select>
            </p>
            <p  id="list_Square">
                <select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++){%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID){%>selected<%} %>><%=listSquareNDB[i].Name %></option><%} %></select>
            </p>

            <p><a class="ip-btl-search-1" href="javascript:void(0)" onclick="doSearch(); return false;">Tìm kiếm</a><a class="ip-btl-datmua" href="<%=ViewPage.GetURL("dat-mua-nha-dat-tai-day") %>">ĐẶT MUA</a></p>
        </form>
    </div>
</div>
<%-- ------------------------------------------------------------ --%>
<script type="text/javascript">
    ...
    if (PriceID > 0)
    {%>
    price_change(<%=PriceID%>);
    <%}
    %>
     <%
    if (ProjectID > 0)
    {%>
    city_change_project(<%=ProjectID%>);
    <%}
    %>
...
function menu_change(menu_id) {
    ...
    function price_change(menu_id) {
        if (menu_id == 1777) {
            document.getElementById("list_Price").innerHTML = '<select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDB != null && i < listPriceNDB.Count; i++){%><option value="<%=listPriceNDB[i].ID %>" <%if (listPriceNDB[i].ID == PriceID){%>selected<%} %>><%=listPriceNDB[i].Name %></option> <%} %></select>';

            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++){%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID){%>selected<%} %>><%=listSquareNDB[i].Name %></option><%} %></select>';
        }
        if (menu_id == 1881) {
            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDCT != null && i < listSquareNDCT.Count; i++){%><option value="<%=listSquareNDCT[i].ID %>" <%if (listSquareNDCT[i].ID == SquareID){%>selected<%} %>><%=listSquareNDCT[i].Name %></option><%} %></select>';
            document.getElementById("list_Price").innerHTML = '<select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDCT != null && i < listPriceNDCT.Count; i++){%><option value="<%=listPriceNDCT[i].ID %>" <%if (listPriceNDCT[i].ID == PriceID){%>selected<%} %>><%=listPriceNDCT[i].Name %></option> <%} %></select>';
        }
        else {
            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++){%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID){%>selected<%} %>><%=listSquareNDB[i].Name %></option><%} %></select>';
            document.getElementById("list_Price").innerHTML = '<select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDB != null && i < listPriceNDB.Count; i++){%><option value="<%=listPriceNDB[i].ID %>" <%if (listPriceNDB[i].ID == PriceID){%>selected<%} %>><%=listPriceNDB[i].Name %></option> <%} %></select>';
