...
<script runat="server">
    List<WebMenuEntity> listType = null, listMenu = null, listCity = null, listProject = null, listPriceNDCT = null, listPriceNDB = null, listSquareNDB = null, listSquareNDCT = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        listPriceNDB = GetByType("PriceNDB");
        listPriceNDCT = GetByType("PriceNDCT");
        listSquareNDB = GetByType("SquareNDB");
        listSquareNDCT = GetByType("SquareNDCT");
...
<form onsubmit="doSearch(); return false;" id="Search">
    <div class="box-timkiem all">
        <div class="title_tk">
            <h2>Tìm kiếm</h2><%if (WebLogin.IsLogin()) 
          {%>
               <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("quan-ly-tin-dang") %>?act=quan-ly-tin-dang">Quản lý Tin</a>
               <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("thanh-vien") %>?act=thong-tin-ca-nhan">Thông tin cá nhân</a>
          <%}%>
        </div>
        <p>
            <input type="text" name="keyword" id="keyword" onblur="onTextBlur('keyword','{RS:Web_SearchText}');" onfocus="onTextFocus('keyword','{RS:Web_SearchText}');" value="{RS:Web_SearchText}" /></p>
        <p>
            <select id="menu_id" onchange="menu_change(this.value),price_change(this.value); return false;">
        ...
        <p id="list_Price">
                <select id="price_id">
                    ...
                    <%for (int i = 0; listPriceNDB != null && i < listPriceNDB.Count; i++)
                      {%><option value="<%=listPriceNDB[i].ID %>" <%if (listPriceNDB[i].ID == PriceID)
                                                                                                                                        {%>selected<%} %>><%=listPriceNDB[i].Name %></option>
                    <%} %></select>
            </p>
            <p id="list_Square">
                <select id="square_id">
                    <option value="0">Diện tích</option>
                    <%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++)
                      {%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID)
                                                                                                                                           {%>selected<%} %>><%=listSquareNDB[i].Name %></option>
                    <%} %></select>
            </p>
         <p><a class="ip-btl-search-1" href="javascript:void(0)" onclick="doSearch(); return false;">Tìm kiếm</a>
         <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("dat-mua-nha-dat-tai-day") %>">Đặt Mua</a>
        <%if (WebLogin.IsLogin()) 
          {%>
               <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("dang-tin") %>">Đăng tin</a>
             <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("thanh-vien/Logout") %>" rel="nofollow">Đăng xuất</a>
          <%}
          else
          {%>
              <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("dang-nhap") %>">Đăng nhập để đăng tin</a>
              <a class="ip-btl-search-1" href="<%=ViewPage.GetURL("dang-ky") %>">Đăng ký</a>
          <%} %>
             </p>
...
function price_change(menu_id) {
    if (menu_id == 1777) {
        document.getElementById("list_Price").innerHTML = '<select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDB != null && i < listPriceNDB.Count; i++)
                                                                                                                        {%><option value="<%=listPriceNDB[i].ID %>" <%if (listPriceNDB[i].ID == PriceID)
                                                                                                                                                                                                                                          {%>selected<%} %>><%=listPriceNDB[i].Name %></option> <%} %></select>';

            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++)
                                                                                                                             {%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID)
                                                                                                                                                                                                                                                  {%>selected<%} %>><%=listSquareNDB[i].Name %></option><%} %></select>';
        }
        if (menu_id == 1881) {
            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDCT != null && i < listSquareNDCT.Count; i++)
                                                                                                                             {%><option value="<%=listSquareNDCT[i].ID %>" <%if (listSquareNDCT[i].ID == SquareID)
                                                                                                                                                                                                                                                     {%>selected<%} %>><%=listSquareNDCT[i].Name %></option><%} %></select>';
            document.getElementById("list_Price").innerHTML = '<select id="price_id"><option value="0">Khoảng giá</option><%for (int i = 0; listPriceNDCT != null && i < listPriceNDCT.Count; i++)
                                                                                                                            {%><option value="<%=listPriceNDCT[i].ID %>" <%if (listPriceNDCT[i].ID == PriceID)
                                                                                                                                                                                                                                                 {%>selected<%} %>><%=listPriceNDCT[i].Name %></option> <%} %></select>';
        }
        else {
            document.getElementById("list_Square").innerHTML = '<select id="square_id"><option value="0">Diện tích</option><%for (int i = 0; listSquareNDB != null && i < listSquareNDB.Count; i++)
                                                                                                                             {%><option value="<%=listSquareNDB[i].ID %>" <%if (listSquareNDB[i].ID == SquareID)
        ...
        //if (menu_id == 1778) {
        //    document.getElementById("price_id").innerHTML = '<select id="listtype_id" class="ddl" ><option value="0">Chọn Danh mục</option></select>';
        //}
        //if (menu_id == 1779) {
        //    document.getElementById("price_id").innerHTML = '<select id="listtype_id" class="ddl" ><option value="0">Chọn Danh mục</option></select>';
        //}
