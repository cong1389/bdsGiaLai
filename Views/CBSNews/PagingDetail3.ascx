...
<div class="box-bds all">
    ...
    <div class="bds-body">
        ...
        <ul class="content_paging2" id="bds-body_<%=VSWID %>">
        </ul>
...
<script type="text/javascript">
    function paging_list_<%=VSWID %>(page) {
        ...
        sLink = '/Tools/GetBSNews.aspx?cmd=tindetail3&MenuID=<%=MenuID%>&isVip=<%=isVip%>&CityID=<%=CityID%>&ProjectID=<%=ProjectID%>&State=<%=State%>&VSWID=<%=VSWID%>&PageSize=<%=PageSize%>&LangID=<%=ViewPage.CurrentLang.ID%>&Page=' + page + '&rnd=' + ranNum;
        AjaxRequest.get(
	        {
	            'url': sLink
