...
<div class="box-bds all">
    ...
    <ul class="list-bds" id="bds-body_<%=VSWID %>">
    </ul>
    <div id="loading"></div>
    <div class="clear"></div>
    <ul class="phantrang" id="<%=VSWID %>"></ul>
</div>
<script type="text/javascript">
    function paging_list_<%=VSWID %>(page) {
        ...
        sLink = '/Tools/GetBSNews.aspx?cmd=mobiletindetail&MenuID=<%=MenuID%>&isVip=<%=isVip%>&CityID=<%=CityID%>&ProjectID=<%=ProjectID%>&State=<%=State%>&PageSize=<%=PageSize%>&VSWID=<%=VSWID%>&LangID=<%=ViewPage.CurrentLang.ID%>&Page=' + page + '&rnd=' + ranNum;
        AjaxRequest.get(
	        {
	            'url': sLink
